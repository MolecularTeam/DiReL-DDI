...
dp_dataset_name = cfg.dataset_name

if 'miner' in dp_dataset_name:
    df_drugs_smiles = pd.read_csv(cfg[dp_dataset_name].drugs_smiles)
    ...
    drug_id_mol_graph_tup = [(id, Chem.MolFromSmiles(smiles.strip())) for id, smiles in
                             zip(df_drugs_smiles['drugbank_id'], df_drugs_smiles['smiles'])]

    drug_to_mol_graph = {id: Chem.MolFromSmiles(smiles.strip()) for id, smiles in
                         zip(df_drugs_smiles['drugbank_id'], df_drugs_smiles['smiles'])}

elif 'ddinter' in dp_dataset_name:
    df_drugs_smiles = pd.read_csv(cfg[dp_dataset_name].drugs_smiles)
...
if 'drugbank' in dp_dataset_name:
    df_all_pos_ddi = pd.read_csv(cfg[dp_dataset_name].ddis)
...
class DrugDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        pos_tri = (pos_h_samples, pos_t_samples, pos_rels, pos_b_samples, pos_h_edge_samples, pos_t_edge_samples)
        ...
        neg_tri = (neg_h_samples, neg_t_samples, neg_rels, neg_b_samples, neg_h_edge_samples, neg_t_edge_samples)
