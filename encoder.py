...
from layers import IntraGraphAttention, InterGraphAttention
...
class DrugEncoder(nn.Module):
    def __init__(self, in_node_features, in_edge_features, hidd_dim, kge_dim, rel_total, heads_out_feat_params,
        ...
        for i, (head_out_feats, n_heads) in enumerate(zip(heads_out_feat_params, blocks_params)):
            block = DrugEncoderBlock(self.hidd_dim, n_heads, head_out_feats, edge_feature, dp)
            self.add_module(f"block{i}", block)
            self.blocks.append(block)

    def encode_pair(self, triples):
        h_data, t_data, rels, b_graph, h_data_edge, t_data_edge = triples
        ...
        h_data.x = F.relu(h_data.x); t_data.x = F.relu(t_data.x)

        h_data.edge_attr = F.relu(self.initial_edge_feature(h_data.edge_attr))
        t_data.edge_attr = F.relu(self.initial_edge_feature(t_data.edge_attr))

        repr_h = []; repr_t = []

        for block in self.blocks:
            h_data, t_data, h_g, t_g = block(h_data, t_data, b_graph, h_data_edge, t_data_edge)
            repr_h.append(h_g); repr_t.append(t_g)

        z_h = torch.stack(repr_h, dim=-2).mean(dim=-2)
        z_t = torch.stack(repr_t, dim=-2).mean(dim=-2)
...
class DrugEncoderBlock(nn.Module):
